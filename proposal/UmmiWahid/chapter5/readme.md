...
Lastly, the study uses the significant variables found to forecast gold prices using the Long Short-Term Memory (LSTM) model and Vector Autoregression (VAR) model. The LSTM and VAR model are suitable for forecasting multivariate time series data. It can show which variables the price of gold has a causal relationship with. As a result, this discovery offers insightful information to policymakers and investors on the ability of integrated economic models to predict market movements and help them make well-informed decisions.
